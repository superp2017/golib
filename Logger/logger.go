...
func GetComLogger() (*ST_Logger, bool) {
	...
	path, err := Config.GetConfigString([]string{"Logger", "Logger"})
	if err != nil {
		return nil, false
	}
	var ok bool
	...
	console, e := Config.GetConfigString([]string{"Logger", "Console"})
	if e != nil {
		console = "false"
	}
	if console == "true" {
...
func New(lout io.Writer, lname string, prefix string, lflag int, llevel int) *ST_Logger {
	msize, err := Config.GetConfigInteger([]string{"Logger", "MaxSize"})
	if err != nil {
		return nil
	}
