...
func enableSession() {
	...
	g_ctx.Sncache = Cache.New(eXPIRE, cLEAR)
	g_ctx_tls.Sncache = Cache.New(eXPIRE, cLEAR)
}
...
func Run() {
	...
	htp, err := Config.GetConfigMap([]string{"Http"})
	if htp != nil && err == nil {
		go http.ListenAndServe(htp["Ip"]+":"+htp["Listen"], g_ctx)
	}

	htps, err := Config.GetConfigMap([]string{"Https"})
